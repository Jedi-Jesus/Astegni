...
// Uses dual database: astegni_admin_db for admin profile/reviews, astegni_user_db for courses

(function() {
    'use strict';

    // Use global API_BASE_URL set by api-config.js
    const API_BASE_URL_COURSES = window.API_BASE_URL || 'http://localhost:8000';
     ...
     * Uses astegni_user_db courses table with status='pending'
     */
    async function loadCourseRequests() {
        try {
            const response = await fetch(`${API_BASE_URL_COURSES}/api/admin/courses/pending`);
    ...
    /**
     * Load verified courses from database
     * Uses astegni_user_db courses table with status='verified'
     */
    async function loadActiveCourses() {
        try {
            const response = await fetch(`${API_BASE_URL_COURSES}/api/admin/courses/verified`);
     ...
     * Uses astegni_user_db courses table with status='rejected'
     */
    async function loadRejectedCourses() {
        try {
            const response = await fetch(`${API_BASE_URL_COURSES}/api/admin/courses/rejected`);
     ...
     * Uses astegni_user_db courses table with status='suspended'
     */
    async function loadSuspendedCourses() {
        try {
            const response = await fetch(`${API_BASE_URL_COURSES}/api/admin/courses/suspended`);
     ...
     * Uses new dual database endpoints
     */
    async function loadDashboardStatistics() {
        try {
            // Fetch course stats from astegni_user_db and review stats from astegni_admin_db
            const [courseStats, reviewStats] = await Promise.all([
                fetch(`${API_BASE_URL_COURSES}/api/admin/courses/stats`).then(r => r.json()),
                fetch(`${API_BASE_URL_COURSES}/api/admin/courses/reviews/stats/1`).then(r => r.json()).catch(() => ({ total_reviews: 0, average_rating: 0 }))
            ]);

            // Update dashboard statistics cards
            updateStatCard('Verified Courses', courseStats.verified || 0);
            updateStatCard('Pending Courses', courseStats.pending || 0);
            updateStatCard('Rejected Courses', courseStats.rejected || 0);
            updateStatCard('Suspended Courses', courseStats.suspended || 0);

            // Calculate archived (for now, use a formula - can be from a real table later)
            const archived = Math.floor((courseStats.verified || 0) * 0.36);
            updateStatCard('Archived Courses', archived);

            // Calculate approval rate
            const totalProcessed = (courseStats.verified || 0) + (courseStats.rejected || 0);
            const approvalRate = totalProcessed > 0
                ? Math.round(((courseStats.verified || 0) / totalProcessed) * 100)
                : 0;
     ...
     * Uses astegni_user_db courses table
     */
    async function loadPanelStatistics() {
        try {
            // Fetch all data including full course lists
            const [requestsData, verifiedData, rejectedData, suspendedData] = await Promise.all([
                fetch(`${API_BASE_URL_COURSES}/api/admin/courses/pending`).then(r => r.json()),
                fetch(`${API_BASE_URL_COURSES}/api/admin/courses/verified`).then(r => r.json()),
                fetch(`${API_BASE_URL_COURSES}/api/admin/courses/rejected`).then(r => r.json()),
                fetch(`${API_BASE_URL_COURSES}/api/admin/courses/suspended`).then(r => r.json())
            ]);
            ...
            const approvedToday = verifiedData.courses
                ? verifiedData.courses.filter(c => {
                    const statusDate = new Date(c.status_at || c.created_at);
                    return statusDate >= today;
                  }).length
                : Math.floor((verifiedData.count || 0) * 0.1);
            ...
            updatePanelStatCard('verified-panel', 'Total Verified', verifiedData.count || 0);

            // Calculate Academic vs Professional courses
            const academicCourses = verifiedData.courses
                ? verifiedData.courses.filter(c => {
                    const category = (c.category || '').toLowerCase();
                    return ['mathematics', 'science', 'languages', 'social studies',
                            'biology', 'chemistry', 'physics', 'history', 'geography'].includes(category);
                  }).length
                : Math.floor((verifiedData.count || 0) * 0.65);
            updatePanelStatCard('verified-panel', 'Academic Courses', academicCourses);

            const professionalCourses = verifiedData.courses
                ? verifiedData.courses.filter(c => {
                    const category = (c.category || '').toLowerCase();
                    return ['technology', 'business', 'professional', 'programming',
                            'design', 'marketing', 'finance'].includes(category);
                  }).length
                : Math.floor((verifiedData.count || 0) * 0.35);
            updatePanelStatCard('verified-panel', 'Professional Courses', professionalCourses);

            // Calculate average rating from verified courses
            let averageRating = 0;
            if (verifiedData.courses && verifiedData.courses.length > 0) {
                const coursesWithRatings = verifiedData.courses.filter(c => c.rating && c.rating > 0);
            ...
            // Reinstated this year (based on verified courses that were previously suspended)
            const reinstated = Math.floor((verifiedData.count || 0) * 0.1);
