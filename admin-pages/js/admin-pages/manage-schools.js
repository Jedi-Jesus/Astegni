...
(function() {
    ...
    // Use existing API_BASE_URL if available (from auth.js), otherwise auto-detect environment
    const SCHOOLS_API_BASE = (typeof API_BASE_URL !== 'undefined')
        ? API_BASE_URL
        : (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1'
            ? 'http://localhost:8000'
            : 'https://api.astegni.com');

    // Ensure SchoolAPI is available (fallback if not loaded)
    if (typeof window.SchoolAPI === 'undefined') {
        console.warn('SchoolAPI not found, creating placeholder...');
        // Create a placeholder that will show appropriate errors
        window.SchoolAPI = {
            getRequestedSchools: async () => { console.error('SchoolAPI not loaded'); return []; },
            getVerifiedSchools: async () => { console.error('SchoolAPI not loaded'); return []; },
            getRejectedSchools: async () => { console.error('SchoolAPI not loaded'); return []; },
            getSuspendedSchools: async () => { console.error('SchoolAPI not loaded'); return []; },
            getSchool: async () => { console.error('SchoolAPI not loaded'); return null; },
            approveSchool: async () => { console.error('SchoolAPI not loaded'); throw new Error('SchoolAPI not loaded'); },
            rejectSchool: async () => { console.error('SchoolAPI not loaded'); throw new Error('SchoolAPI not loaded'); },
            suspendSchool: async () => { console.error('SchoolAPI not loaded'); throw new Error('SchoolAPI not loaded'); },
            reconsiderSchool: async () => { console.error('SchoolAPI not loaded'); throw new Error('SchoolAPI not loaded'); },
            reinstateSchool: async () => { console.error('SchoolAPI not loaded'); throw new Error('SchoolAPI not loaded'); },
            deleteSchool: async () => { console.error('SchoolAPI not loaded'); throw new Error('SchoolAPI not loaded'); }
        };
    }
    ...
    function updateStatistics() {
        ...
        updateStatCard('Total Schools', totalCount, null);
    ...
    window.reconsiderSchoolFromTable = async function(schoolId) {
        ...
        try {
            // Use unified reconsider endpoint for all cases
            await SchoolAPI.reconsiderSchool(schoolId);
            showNotification('School moved to pending for reconsideration', 'success');

            // Reload appropriate tables based on source
            const table = currentSchool ? currentSchool.table : 'rejected';
            if (table === 'rejected') {
                await loadRejectedSchools();
            }
            await loadRequestedSchools();
    ...
    // ============================================
    // NOTE: These functions are defined in manage-schools-standalone.js
    // which handles hero_title, hero_subtitle, location, allow_location, display_location
    // DO NOT override them here - just add any additional functionality if needed
    // ============================================
    ...
    async function loadRecentReviews() {
        try {
            ...
            // Fetch admin profile to get admin_id using new endpoint
            const profileResponse = await fetch(`${SCHOOLS_API_BASE}/api/admin/schools/profile/by-email/${encodeURIComponent(adminEmail)}`);
            ...
            // Fetch recent reviews from admin_reviews using new endpoint
            const reviewsResponse = await fetch(`${SCHOOLS_API_BASE}/api/admin/schools/reviews/recent?limit=5&admin_id=${adminId}`);
            ...
            // Filter for school-related reviews if department field exists, otherwise show all
            const schoolReviews = allReviews.filter(review =>
                !review.department || review.department === 'schools' || review.review_type === 'school'
            );
                ...
                if (scrollContainer) {
                    scrollContainer.innerHTML = `
                        <div class="text-center text-gray-500 py-8">
                            ...
                            <p class="font-medium">No schools yet</p>
                            <p class="text-sm">Schools will appear here as they are added</p>
                        </div>
                    `;
                }
                return;
