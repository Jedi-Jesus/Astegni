...
@router.get("/api/view-student/{student_user_id}/documents")
async def get_view_student_documents(
    student_user_id: int,
    document_type: Optional[str] = None
):
    """
    PUBLIC endpoint to get student documents for view-student.html

    - Used by view-student.html to display student's documents
    ...
    """
    try:
        print(f"[VIEW STUDENT DOCS] Getting documents for user_id: {student_user_id}, type: {document_type}")
        ...
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                ...
                if not result:
                    print(f"[VIEW STUDENT DOCS] Student profile not found for user_id: {student_user_id}")
                    return []  # Return empty list gracefully

                student_id = result['id']
                print(f"[VIEW STUDENT DOCS] Found student_id: {student_id}")
                ...
                documents = cur.fetchall()
                print(f"[VIEW STUDENT DOCS] Found {len(documents)} documents")
        ...
        print(f"[VIEW STUDENT DOCS] Error fetching documents: {e}")
        import traceback
        traceback.print_exc()
