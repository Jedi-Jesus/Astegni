...
def get_user_display_info(conn, user_id: int, profile_type: str, profile_id: int) -> dict:
    ...
    # Try to get profile-specific info (username only, profile_picture is in users table)
    ...
    try:
        ...
        conn.rollback()
        print(f"[Chat API] Warning: Failed to get profile-specific info for {profile_type} {profile_id}: {e}")
...
def get_user_privacy_settings(conn, profile_id: int, profile_type: str) -> dict:
    ...
    try:
        ...
        # Rollback on error and return defaults
        conn.rollback()
        print(f"[Chat API] Warning: Failed to get privacy settings for {profile_type} {profile_id}: {e}")
        return {
            "who_can_message": "everyone",
            "read_receipts": True,
            "online_status": True,
                ...
                if student_row and student_row['parent_id']:
                    ...
                    for parent_profile_id in parent_profile_ids:
                        if parent_profile_id:
                            ...
                            cur.execute("""
                                SELECT pp.id, pp.user_id, pp.username,
                            ...
                            if parent_row and parent_row['user_id'] not in existing_user_ids:
                                ...
                                avatar = parent_row['user_picture']
                ...
                if parent_row and parent_row['children_ids']:
                    ...
                    for child_profile_id in children_profile_ids:
                        if child_profile_id:
                            ...
                            # NOTE: profile_picture now read from users table
                            cur.execute("""
                                SELECT sp.id, sp.user_id, sp.username,
                                       u.first_name, u.father_name, u.profile_picture as user_picture
                        ...
                        for parent_profile_id in student['parent_ids']:
                            if parent_profile_id:
                                ...
                                if parent_row and parent_row['user_id'] not in existing_user_ids:
                                    ...
                                    parent_avatar = parent_row['user_picture']
            ...
            if profile_type == 'tutor':
                ...
                for req in session_requests:
                    if req['requester_id'] not in existing_user_ids:
                        ...
                        # Get requester's profile info (username only, profile_picture is in users table)
                        requester_profile_id = None
                        if req['requester_type'] == 'student':
                            cur.execute("SELECT id, username FROM student_profiles WHERE user_id = %s", (req['requester_id'],))
                            ...
                            cur.execute("SELECT id, username FROM parent_profiles WHERE user_id = %s", (req['requester_id'],))
                            profile_row = cur.fetchone()
                            if profile_row:
                                requester_profile_id = profile_row['id']
                                if profile_row.get('username'):
        ...
        if filter_type in ['all', 'direct', None]:
            ...
            for inv in sent_invitations:
                ...
                if invited_user_id and should_add:
                    ...
                    # Get invited user's parent profile (username only, profile_picture is in users table)
                    invited_profile_id = None
                    cur.execute("SELECT id, username FROM parent_profiles WHERE user_id = %s", (invited_user_id,))
                    profile_row = cur.fetchone()
                    if profile_row:
            ...
            for inv in received_invitations:
                ...
                if should_add:
                    ...
                    cur.execute("SELECT id, username FROM parent_profiles WHERE user_id = %s", (inviter_user_id,))
                    profile_row = cur.fetchone()
                    if profile_row:
        ...
        if filter_type in ['all', 'direct', None]:
            ...
            for inv in sent_child_invitations:
                ...
                if invited_user_id and should_add:
                    ...
                    # Get invited user's student profile (username only, profile_picture is in users table)
                    invited_profile_id = None
                    cur.execute("SELECT id, username FROM student_profiles WHERE user_id = %s", (invited_user_id,))
                    profile_row = cur.fetchone()
                    if profile_row:
                        invited_profile_id = profile_row['id']
                        if profile_row.get('username'):
            ...
            for inv in received_child_invitations:
                ...
                if should_add:
                    ...
                    # Get inviter's parent profile (username only, profile_picture is in users table)
                    inviter_profile_id = None
                    cur.execute("SELECT id, username FROM parent_profiles WHERE user_id = %s", (inviter_user_id,))
                    profile_row = cur.fetchone()
                    if profile_row:
