    ...
    if (APP_STATE.currentUser?.roles?.length > 1) {
        ...
        const switcher = `
            <select onchange="switchRole(this.value)">
                ${APP_STATE.currentUser.roles.map(role => 
                    `<option value="${role}" ${role === APP_STATE.userRole ? 'selected' : ''}>${role}</option>`
                ).join('')}
            </select>
        `;
...
function updateUIForLoggedInUser() {
    ...
    if (profilePic && APP_STATE.userRole) {
        const userAvatar = getUserAvatar(APP_STATE.userRole);
        profilePic.src = userAvatar;
        profilePic.alt = `${APP_STATE.userRole} avatar`;
        profilePic.style.cursor = 'pointer';
        
        // Add error handler for profile pic
        profilePic.onerror = () => {
        ...
        profilePic.onclick = () => openAvatarSelection();
    }
...
// Get saved avatar or assign new one
function getUserAvatar(role) {
    let savedAvatar = localStorage.getItem('userAvatar');
    
    // Check if saved avatar exists and is valid
    if (savedAvatar) {
        // If it's a local path, check if it's accessible
        if (savedAvatar.startsWith('pictures/') || savedAvatar.startsWith('../')) {
            // Test if image loads
            const img = new Image();
            img.onerror = () => {
                // If local image fails, get new avatar
                savedAvatar = getRandomRoleAvatar(role);
...
function updateProfileLink(role) {
    const profileUrl = PROFILE_URLS[role] || 'my-profile-tier-1/user-profile.html';
...
function openAvatarSelection() {
    ...
    const avatars = ROLE_AVATAR_COLLECTIONS[APP_STATE.userRole] || [];
    ...
    avatars.forEach((avatarUrl, index) => {
        ...
        img.onclick = () => selectAvatar(avatarUrl, img);
...
function saveAvatarChoice() {
    if (selectedAvatarUrl) {
        saveUserAvatar(selectedAvatarUrl);
