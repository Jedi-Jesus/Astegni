"""
Migration: Add country column to cpi_settings table
Adds GPS-based country detection support to CPI pricing
"""
...
def migrate():
    """Add country column to cpi_settings table"""

    with engine.connect() as conn:
        print("Starting migration: Add country column to cpi_settings table")
        ...
        try:
            ...
            conn.execute(text("""
                CREATE INDEX IF NOT EXISTS idx_cpi_settings_country
            ...
            print("\nCPI settings now support country-specific pricing:")
...
def verify():
    ...
    with engine.connect() as conn:
        ...
        result = conn.execute(text("""
            SELECT indexname
            FROM pg_indexes
            WHERE tablename = 'cpi_settings'
            AND indexname = 'idx_cpi_settings_country'
        """))

        if result.fetchone():
            print("[OK] Index 'idx_cpi_settings_country' exists")
        else:
