...
# ============================================
# REMOVED: Legacy /api/advertiser/campaigns* + /analytics endpoints
# ============================================
# A set of endpoints used to live here (create/list/get/update/delete campaigns,
# update metrics, get analytics, and a _update_advertiser_stats helper). They all
# referenced a SQLAlchemy `AdCampaign` model that no longer exists in models.py,
# so they 500'd on call. Production logs (7 days) show zero hits — the live
# traffic goes through advertiser_brands_endpoints.py which uses the real
# campaign_profile table via raw SQL. Removed to prevent future confusion.
# If a feature listed below is needed, build it against campaign_profile / brand_profile:
#   - GET    /api/advertiser/analytics         (was at this line range)
#   - GET    /api/advertiser/campaigns         (use /api/advertiser/brands/{id}/campaigns instead)
#   - POST   /api/advertiser/campaigns         (use /api/advertiser/brands/{id}/campaigns instead)
#   - GET    /api/advertiser/campaigns/{id}    (handled by brands router today)
#   - PUT    /api/advertiser/campaigns/{id}    (handled by brands router today)
#   - PUT    /api/advertiser/campaigns/{id}/metrics  (no replacement; metrics computed elsewhere)
# Removed in commit immediately following 154bd9a.
