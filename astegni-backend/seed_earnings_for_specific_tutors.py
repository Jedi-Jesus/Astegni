"""
Seed Earnings and Investments Data for Specific Tutors
Seeds data for user_id 85 and 86
"""
...
def seed_tutor_earnings(tutor_user_id, tutor_profile_id, student_users):
    ...
    try:
        print(f"\n{'='*70}")
        print(f"Seeding data for User ID: {tutor_user_id}, Tutor Profile ID: {tutor_profile_id}")
        print(f"{'='*70}")
        ...
        for i in range(25):
            ...
            """, (
                ...
                2000 + (tutor_user_id * 100) + i,  # Unique session IDs
                round(amount, 2),
        ...
        conn.commit()
        print(f"\nSUCCESS: Data seeded for tutor {tutor_user_id}!")
...
def main():
    """Seed data for both tutor_id 85 and 86"""
    conn = psycopg.connect(DATABASE_URL)
    cur = conn.cursor()
    ...
    try:
        ...
        if not student_users:
            ...
            for i in range(15):
                ...
                email = f"student{i+100}@astegni.et"
                cur.execute("""
        ...
        # Get tutor profiles for user_id 85 and 86
        cur.execute("""
            SELECT u.id, u.email, tp.id as tutor_profile_id
            FROM users u
        ...
        if not tutors:
            print("ERROR: No tutor profiles found for user_id 85 and 86")
            print("Please ensure these users have tutor profiles.")
            return

        print(f"\nFound {len(tutors)} tutors to seed:")
        for tutor in tutors:
            print(f"  - User ID {tutor[0]}: {tutor[1]} (Tutor Profile ID: {tutor[2]})")

        # Seed data for each tutor
        for tutor in tutors:
            user_id = tutor[0]
            tutor_profile_id = tutor[2]
            seed_tutor_earnings(user_id, tutor_profile_id, student_users)

        print("\n" + "="*70)
        print("ALL DONE! Earnings and investments seeded for both tutors!")
        print("="*70)
        print("\nLogin credentials:")
        for tutor in tutors:
            print(f"  - {tutor[1]}")
