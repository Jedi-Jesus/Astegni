// ============================================
// PARENT SESSION REQUESTS MANAGER
// Manages display and filtering of session requests made by the parent
// ============================================

const ParentSessionRequestsManager = {
    currentFilter: 'all',
    allRequests: [],
     ...
     */
    async initialize() {
        console.log('[ParentSessionRequestsManager] Initializing...');
        await this.loadMyRequests();
    },

    /**
     * Load all session requests made by the current parent
     */
    async loadMyRequests() {
        const container = document.getElementById('parent-session-requests-list');
        if (!container) {
            console.warn('[ParentSessionRequestsManager] Container not found');
            return;
        }
        ...
        try {
            ...
            console.log('[ParentSessionRequestsManager] Loaded requests:', requests.length);
            ...
            if (requests.length === 0) {
                container.innerHTML = `
                    <div class="card p-8 text-center text-gray-500">
                        ...
                        <p class="mb-4">You haven't requested any tutoring sessions for your children yet.</p>
                        <a href="../branch/find-tutors.html" class="btn-primary inline-block">
                            Find Tutors for My Children
                        </a>
                    </div>
                `;
                return;
            ...
            console.error('[ParentSessionRequestsManager] Error loading requests:', error);
            container.innerHTML = `
                <div class="card p-6 text-center text-red-500">
                    <i class="fas fa-exclamation-triangle text-3xl mb-3"></i>
                    <p class="font-semibold">Failed to load session requests</p>
                    <p class="text-sm mt-2">${error.message}</p>
                    <button onclick="ParentSessionRequestsManager.loadMyRequests()" class="btn-primary mt-4">
                        Try Again
                    </button>
                </div>
            `;
        ...
        return `
            <div class="card p-6 mb-4 hover:shadow-lg transition-shadow">
                ...
                <div class="grid grid-cols-2 gap-4 mb-4 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
                    ...
                    <div>
                        <p class="text-sm text-gray-500 mb-1">Child</p>
                ...
                <div class="flex gap-2 mt-4">
                    <button onclick="ParentSessionRequestsManager.viewRequestDetails(${request.id})"
                            class="btn-secondary flex-1">
                        <i class="fas fa-eye mr-2"></i>
                        View Details
                    </button>
                    ${request.status === 'accepted' ? `
                    <button onclick="ParentSessionRequestsManager.contactTutor(${request.tutor_id})"
    ...
    viewRequestDetails(requestId) {
        ...
        console.log('[ParentSessionRequestsManager] Request details:', request);

        alert(`Request Details:\n\n` +
              `Tutor: ${request.requester_name}\n` +
              `Package: ${request.package_name}\n` +
              `Status: ${request.status}\n` +
              `Child: ${request.student_name} (${request.student_grade})\n` +
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.switchPanel = function(panelId) {
        ...
        if (panelId === 'my-requests') {
            ParentSessionRequestsManager.initialize();
        }
    };
});

// Export to window for onclick handlers
window.ParentSessionRequestsManager = ParentSessionRequestsManager;
