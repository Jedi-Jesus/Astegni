...
function openCoverUploadModal() {
    openModal('cover-upload-modal');
    initializeUploadArea('cover');
}

function openProfileUploadModal() {
    openModal('profile-upload-modal');
    initializeUploadArea('profile');
}
...
function initializeUploadArea(type) {
    ...
    uploadArea.addEventListener('dragleave', () => {
        uploadArea.style.borderColor = 'rgba(var(--border-rgb), 0.3)';
    });
...
function handleFileSelect(file, type) {
    ...
    reader.onload = (e) => {
        ...
        document.querySelector(`#${type}-upload-area .upload-placeholder`).classList.add('hidden');
    };
...
function uploadCover() {
    ...
    if (preview.src && preview.src !== '#') {
        document.getElementById('hero-cover').src = preview.src;
        AppState.user.coverPic = preview.src;
        notifications.show('Cover photo updated!', 'success');
        closeModal('cover-upload-modal');
    }
}
...
function uploadProfile() {
    ...
    if (preview.src && preview.src !== '#') {
        document.getElementById('hero-avatar').src = preview.src;
        document.getElementById('nav-profile-pic').src = preview.src;
        AppState.user.profilePic = preview.src;
        notifications.show('Profile picture updated!', 'success');
        closeModal('profile-upload-modal');
    }
}
...
function saveCampaign() {
    ...
    notifications.show('Campaign created successfully!', 'success');
