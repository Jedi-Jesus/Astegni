        ...
        function initDashboard() {
            try {
                ...
                // searchRejectedCampaignContent(); // Commented out - function doesn't exist
                updateNotifications();
                searchSchools();
                ...
                showTab('campaigns'); // Default to campaigns tab
...
function uploadCoverPic() {
        ...
        function openCoverPicModal() {
            document.getElementById('cover-pic-modal').classList.remove('hidden');
        }
        ...
        function uploadCoverPic() {
            ...
            if (fileInput.files.length > 0) {
                ...
                const reader = new FileReader();
                reader.onload = function(e) {
                    document.getElementById('admin-cover-pic').src = e.target.result;
                    closeCoverPicModal();
                };
                reader.readAsDataURL(file);
            }
        ...
        function uploadProfilePic() {
            ...
            if (fileInput.files.length > 0) {
                ...
                reader.onload = function(e) {
                    ...
                    document.getElementById('profile-btn').querySelector('img').src = e.target.result;
                    document.getElementById('mobile-profile-btn').querySelector('img').src = e.target.result;
                    closeProfilePicModal();
                };
                reader.readAsDataURL(file);
            }
        ...
        function checkNotifications() {
            ...
            if (mobileNotificationDot) {
                mobileNotificationDot.classList.toggle('hidden', !hasNotifications);
            }
        }

        function searchRejectedCampaigns() {
            // Search rejected campaigns functionality
            const searchTerm = (document.getElementById('rejected-campaign-search')?.value || '').toLowerCase();
            const table = document.getElementById('rejected-campaign-table');
            if (table) {
                table.innerHTML = '';
                Object.values(campaigns).forEach(campaign => {
                    if (campaign.rejectionReason && (campaign.name.toLowerCase().includes(searchTerm) || campaign.rejectionReason.toLowerCase().includes(searchTerm))) {
                        ...
                        row.innerHTML = `
                            ...
                            <td class="p-2">${campaign.adContent.description}</td>
                            <td class="p-2">${campaign.rejectionReason}</td>
                            <td class="p-2">
                                <button onclick="reviewCampaign(${campaign.id}, true)" class="px-2 py-1 cta-button">Re-review</button>
                            </td>
                        `;
                        table.appendChild(row);
                    }
        ...
        function searchAstegniComments() {
            // Search and display Astegni comments
            const searchTerm = (document.getElementById('comment-search')?.value || '').toLowerCase();
            const container = document.getElementById('astegni-comment-container');
            if (container) {
                container.innerHTML = '';
                Object.values(astegniComments).forEach(comment => {
                    if (comment.userName.toLowerCase().includes(searchTerm) || comment.content.toLowerCase().includes(searchTerm)) {
                        ...
                        commentCard.innerHTML = `
                            <h4>${comment.userName} (${comment.userType})</h4>
                            <p>${comment.content}</p>
                            <p><small>${comment.date}</small></p>
                            <div class="rating-breakdown">
                                <span>Usability: ${comment.usabilityRating}/5</span>
                                <span>Culture: ${comment.cultureRating}/5</span>
                                <span>Management: ${comment.managementRating}/5</span>
                            </div>
                        `;
