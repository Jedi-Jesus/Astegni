...
updateSidebarPosition() {
    ...
    if (footerRect.top < windowHeight) {
        ...
        const easedOffset = this.easeInOutCubic(
            visibleFooterHeight / footerRect.height
        ) * visibleFooterHeight;
            ...
            <div class="modal-content notifications-modal">
                <div class="modal-header">
                    <h2>
                        ...
                        Notifications
                        <span class="notification-count">${this.notifications.filter((n) => n.unread).length
            } new</span>
                    </h2>
        ...
        return this.notifications
            ...
            <div class="notification-item ${notif.unread ? "unread" : ""
                ...
                <div class="notification-actions">
                    ...
                    <button class="notification-action-btn" onclick="window.notificationsManager.dismiss(${notif.id
                    })">×</button>
                </div>
            </div>
        `
...
const AIInsights = {
    ...
    showAIInsights() {
        ...
        document.querySelector('.sidebar-btn[data-content="ai-insights"]').classList.add('active');
    },
...
class CommunityManager {
    ...
    loadFollowers(container) {
        ...
        container.innerHTML = followers.map((user, index) => `
            <div class="follower-card animated-entry" style="animation-delay: ${index * 0.05}s">
                ...
                <div class="follower-actions">
                    ...
                    <button class="btn-connect" onclick="window.communityManager.connect('${user.id}')">Connect</button>
                </div>
            </div>
        `).join("");
    }
    ...
    loadFollowing(container) {
        ...
        container.innerHTML = following.map((user, index) => `
            <div class="follower-card animated-entry" style="animation-delay: ${index * 0.05}s">
                <div class="follower-header">
                    <img src="${user.avatar}" alt="${user.name}" class="follower-avatar">
                    ${user.isOnline ? '<span class="online-badge"></span>' : ""}
                ...
                <div class="follower-actions">
                    <button class="btn-unfollow" onclick="window.communityManager.unfollow('${user.id}')">Unfollow</button>
                    <button class="btn-connect" onclick="window.communityManager.connect('${user.id}')">Message</button>
                </div>
            </div>
        `).join("");
    }
    ...
    const groups = window.contentLoader ? window.contentLoader.getGroupsByType('all') : [];
        container.innerHTML = groups.map((group, index) => `
            <div class="group-card animated-entry" style="animation-delay: ${index * 0.05}s">
                ...
                <button class="btn-primary" onclick="window.communityManager.joinGroup('${group.id}')">Join Group</button>
            </div>
        `).join("");
    }
    ...
    loadClubs(container) {
        ...
        container.innerHTML = clubs.map((club, index) => `
            <div class="club-card animated-entry" style="animation-delay: ${index * 0.05}s">
