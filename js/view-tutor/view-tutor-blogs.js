...
/**
 * Fetch tutor blogs from API
 */
async function fetchTutorBlogs(tutorUserId) {
    try {
        // Fetch blogs for this specific tutor profile
        const response = await fetch(`${API_BASE_URL}/api/blogs?role=tutor&limit=50`);
        ...
        // Filter to only show blogs from this specific tutor profile
        const tutorBlogs = allBlogs.filter(blog => blog.profile_id === tutorUserId);
...
/**
 * Load and render tutor blogs
 */
async function loadTutorBlogs(tutorUserId) {
    const container = document.getElementById('tutor-blogs-container');
    if (!container) return;

    try {
        console.log('Loading blogs for tutor user ID:', tutorUserId);

        const blogs = await fetchTutorBlogs(tutorUserId);
        ...
        if (blogs.length === 0) {
            container.innerHTML = `
                <div style="text-align: center; padding: 4rem 2rem;">
                    ...
                    <p style="color: var(--text-secondary);">This tutor hasn't written any blog posts yet.</p>
                </div>
            `;
            return;
        }
        ...
        console.log(`Loaded ${blogs.length} blog post(s) for tutor ${tutorUserId}`);

    } catch (error) {
        console.error('Error loading tutor blogs:', error);
        container.innerHTML = `
            <div style="text-align: center; padding: 4rem 2rem;">
                ...
                <button onclick="loadTutorBlogs(${tutorUserId})"
...
window.loadTutorBlogs = loadTutorBlogs;
