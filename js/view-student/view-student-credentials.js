 ...
 * Handles fetching and displaying achievements and academic credentials
 * Uses the /api/view-student/{student_profile_id}/credentials endpoint
 *
 * Data Flow:
 * 1. Frontend passes student_profiles.id
 * 2. Backend: student_profiles.id -> student_profiles.user_id -> credentials.uploader_id
 * 3. credentials.uploader_id = users.id (NOT student_profiles.id)
 */
 ...
 *
 * Backend Flow:
 * - Takes student_profiles.id as parameter
 * - Looks up users.id from student_profiles.user_id
 * - Queries credentials WHERE uploader_id = users.id AND uploader_role = 'student'
 *
 * @param {number} studentProfileId - The student_profiles.id
 * @param {string} credentialType - Optional filter: 'achievement', 'academic_certificate'
 */
...
/**
 * Render academic credentials section (certifications)
 */
function renderCertifications(credentials) {
    ...
    if (credentials.length === 0) {
        section.innerHTML = `
            <div class="col-span-full card p-6 text-center text-gray-500">
                <i class="fas fa-graduation-cap text-3xl mb-3" style="color: #3b82f6; opacity: 0.5;"></i>
                <p>No academic credentials yet</p>
                <p class="text-sm mt-2">This student hasn't added any academic credentials yet</p>
            </div>
        `;
        return;
    }

    // Render academic credential cards
        ...
        return `
            <div class="card" style="padding: 1.5rem; border-radius: 12px; background: var(--card-bg); border: 1px solid var(--border-color); transition: var(--transition); box-shadow: var(--shadow-sm);">
                ...
                <div style="display: flex; align-items: center; gap: 1rem; margin-bottom: 1.25rem; padding-bottom: 1rem; border-bottom: 1px solid var(--border-color);">
                    ...
                    <div style="flex: 1;">
                        <h4 style="font-weight: 700; font-size: 1.125rem; margin: 0 0 0.25rem 0; color: var(--heading);">
                            ${cred.title || 'Untitled Credential'}
                        </h4>
                        <div style="display: flex; align-items: center; gap: 0.75rem; flex-wrap: wrap;">
                            <span style="display: inline-flex; align-items: center; gap: 0.25rem; font-size: 0.875rem; color: var(--text-secondary);">
...
async function updateCredentialCounts(studentProfileId) {
    try {
        ...
        // Update academic credentials count
        ...
        if (certificationsCountEl) {
            ...
            certificationsCountEl.textContent = count === 0 ? 'No Credentials' : count === 1 ? '1 Credential' : `${count} Credentials`;
        }
        ...
        if (certificationsCountEl) certificationsCountEl.textContent = '0 Credentials';
    }
}
