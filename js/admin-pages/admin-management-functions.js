...
const sampleAdmins = [
    ...
    },
    {
        ...
        permissions: ['manage-users', 'manage-tutor-documents'],
        totalActions: 678,
        loginCount: 189
    }
];
...
async function handleAdminInvitation(event) {
    ...
    const name = document.getElementById('invite-admin-name').value;
    const email = document.getElementById('invite-admin-email').value;
    const role = document.getElementById('invite-admin-role').value;
    const message = document.getElementById('invite-admin-message').value;

    // Get selected permissions
    const permissions = [];
    document.querySelectorAll('#invite-admin-modal input[type="checkbox"]:checked').forEach(cb => {
    ...
    try {
        ...
        if (response.ok) {
            ...
            alert(`Invitation sent successfully to ${email}!\n\nInvitation link: ${data.invitationLink || 'Sent via email'}\nExpires in 7 days.`);
