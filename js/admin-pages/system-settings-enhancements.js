...
// ============================================
// 4. Campaign Package Drag-and-Drop
// ============================================

let draggedPackage = null;

function initializeCampaignPackageDragDrop() {
    const grid = document.getElementById('campaign-packages-grid');
    if (!grid) return;

    // Make grid items draggable
    const updatePackageCards = () => {
        const packages = grid.querySelectorAll('.campaign-package-card');
        ...
        packages.forEach(packageCard => {
            ...
            packageCard.addEventListener('drop', function(e) {
                ...
                if (draggedPackage !== this) {
                    ...
                    const allPackages = Array.from(grid.querySelectorAll('.campaign-package-card'));
...
async function saveCampaignPackageOrder() {
    try {
        const grid = document.getElementById('campaign-packages-grid');
        const packages = Array.from(grid.querySelectorAll('.campaign-package-card'));
        ...
        const response = await fetch(`${window.API_BASE_URL}/api/admin/campaign-packages/reorder`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'
...
// ============================================
// 5. Enhanced Campaign Package Modal Handling
// ============================================

// Get selected package label
function getSelectedPackageLabel() {
    const selectedRadio = document.querySelector('input[name="campaign-package-label"]:checked');
    return selectedRadio ? selectedRadio.value : 'none';
}

// Set package label in modal
function setPackageLabel(label) {
    const radio = document.querySelector(`input[name="campaign-package-label"][value="${label}"]`);
    if (radio) {
        radio.checked = true;
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    // Initialize drag-and-drop for campaign packages
    setTimeout(() => {
        initializeCampaignPackageDragDrop();
    }, 500);
