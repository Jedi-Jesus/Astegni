...
(function() {
    ...
    async function loadLiveCourseRequests() {
        try {
            ...
            // Fetch from all tables to show mixed statuses
            const [requests, active, rejected, suspended] = await Promise.all([
                fetch(`${API_BASE_URL}/api/course-management/requests`).then(r => r.json()).catch(() => ({courses: []})),
                fetch(`${API_BASE_URL}/api/course-management/active`).then(r => r.json()).catch(() => ({courses: []})),
                fetch(`${API_BASE_URL}/api/course-management/rejected`).then(r => r.json()).catch(() => ({courses: []})),
                fetch(`${API_BASE_URL}/api/course-management/suspended`).then(r => r.json()).catch(() => ({courses: []}))
            ]);
