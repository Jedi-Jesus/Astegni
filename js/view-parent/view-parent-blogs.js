...
/**
 * Fetch parent blogs from API
 */
async function fetchParentBlogs(parentUserId) {
    try {
        // Fetch blogs for this specific parent profile
        const response = await fetch(`${API_BASE_URL}/api/blogs?role=parent&limit=50`);
        ...
        // Filter to only show blogs from this specific parent profile
        const parentBlogs = allBlogs.filter(blog => blog.profile_id === parentUserId);
...
/**
 * Load and render parent blogs
 */
async function loadParentBlogs(parentUserId) {
    const container = document.getElementById('parent-blogs-container');
    if (!container) return;

    try {
        console.log('Loading blogs for parent user ID:', parentUserId);

        const blogs = await fetchParentBlogs(parentUserId);
        ...
        if (blogs.length === 0) {
            container.innerHTML = `
                <div style="text-align: center; padding: 4rem 2rem;">
                    ...
                    <p style="color: var(--text-secondary);">This parent hasn't written any blog posts yet.</p>
                </div>
            `;
            return;
        }
        ...
        console.log(`Loaded ${blogs.length} blog post(s) for parent ${parentUserId}`);

    } catch (error) {
        console.error('Error loading parent blogs:', error);
        container.innerHTML = `
            <div style="text-align: center; padding: 4rem 2rem;">
                ...
                <button onclick="loadParentBlogs(${parentUserId})"
...
window.loadParentBlogs = loadParentBlogs;
