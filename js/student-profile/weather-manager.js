// ============================================
// WEATHER MANAGER - STUDENT PROFILE
// Handles weather widget with theme switching
// ============================================
...
window.weatherManager = {
    ...
    changeTheme: function (theme) {
        ...
        if (this.themes[theme]) {
            ...
            localStorage.setItem('student-weather-theme', theme);
        }
    },
    ...
    applyCustomColors: function () {
        ...
        localStorage.setItem('student-weather-theme', 'custom');
        localStorage.setItem('student-weather-custom-start', startColor);
        localStorage.setItem('student-weather-custom-end', endColor);
    },
    ...
    // Initialize weather manager - load saved theme
    init: function() {
        const savedTheme = localStorage.getItem('student-weather-theme');
        if (savedTheme) {
            if (savedTheme === 'custom') {
                const startColor = localStorage.getItem('student-weather-custom-start');
                const endColor = localStorage.getItem('student-weather-custom-end');
...
function updateMarketValue(element, newValue, oldValue) {
    ...
    if (rateElement) {
        rateElement.textContent = newValue.toFixed(2);
        rateElement.style.color = isPositive ? '#10b981' : '#ef4444';
    }
